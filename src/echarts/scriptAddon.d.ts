...
> {
  /**
   * 当组件第一次初始化时调用的函数 (可选)
   * @param {ECharts} myChart echarts 实例，详见 echarts 的 API 文档
   * @param {AttributesType} addonAttributes <$echarts> 控件传入的的所有参数，是 object
   * @param {Widget} [echartsWidget] <$echarts> 控件实例，可能为 undefiend
  ...
  /**
   * 判断是否需要刷新组建 (可选)
   * @param {StateType} state 组件的状态，就是 onMount 返回的那个
   * @param {IChangedTiddlers} changedTiddlers 刷新是由 TW 系统监听到有条目发生变化才会触发的，这是一个包含所有变更条目标题的字符串数组
   * @param {Record<string, true>} changedAttributes 哪些参数被改变了，包括$开头的参数
   * @return {boolean} 如果需要刷新就返回 true，反之
   *
